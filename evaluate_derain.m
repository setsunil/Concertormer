...
%% matlab -nodesktop -nosplash -r "evaluate_derain('FLASH_v45_derain_310000')"

function [] = evaluate_derain(folder)
%clc;%lose all;clear all;

datasets = {'Test100', 'Rain100H', 'Rain100L', 'Test2800'};
...
delete(gcp('nocreate'))
parpool('local',6);
fileID = fopen(strcat('./results/', folder, '/metrics.txt'), 'a');
for idx_set = 1:num_set
    file_path = strcat('./results/', folder, '/visualization/', datasets{idx_set}, '/');
    gt_list = [dir(strcat(file_path,'*_gt.jpg')); dir(strcat(file_path,'*_gt.png'))];
    ...
    if img_num > 0
        parfor j = 1:img_num
           ...
           image_name = erase(gt_name, '_gt');

           input = imread(strcat(file_path,image_name));
           gt = imread(strcat(file_path, gt_name));
    ...
    fprintf(fileID, 'For %s dataset PSNR: %f SSIM: %f\n', datasets{idx_set}, qm_psnr, qm_ssim);
...
fprintf(fileID, 'For all datasets PSNR: %f SSIM: %f\n', psnr_alldatasets/num_set, ssim_alldatasets/num_set);
delete(gcp('nocreate'))
toc
fclose(fileID);
end
