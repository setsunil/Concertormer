...
class PatchMatch(nn.Module):
    ...
    def forward(self, left_features, right_features):
        ...
        device = left_features.get_device()
        if self.propagation_type == "faster_filter_3_propagation":
...
if __name__ == '__main__':
    ...
    from basicsr.models.archs.PatchMatch.FeatureExtFast import feature_extraction
    config = obj(config)
    f = feature_extraction()
    n = PatchMatch(config.patch_match_args)
    a = torch.randn(1,3,6,6).cuda()
    b = torch.randn(1,3,6,6).cuda()
    left_features, right_features, neighbour_extraction_filter = f(a, b)
    offset_x, offset_y, x_coordinate, y_coordinate = n(left_features, right_features)
    print(f'offset_x: {offset_x}, offset_y: {offset_y}, x_coordinate: {x_coordinate}, y_coordinate: {y_coordinate}')
